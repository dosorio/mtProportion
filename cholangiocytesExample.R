...
dF <- dF[downloadedCells$CellTypes %in% 'Cholangiocytes',]
...
E <- plotEnrichment(KEGG$Apoptosis, FC) +
  ...
  labs(title = 'Apoptosis', subtitle = paste0('6 vs 3 | NES = ',round(PValue$NES,2), ' | P = ', formatC(PValue$padj, format = 'e', digits = 2)), tag = 'E')

DE <- FindMarkers(downloadedCells, ident.1 = '8', ident.2 = '3', test.use = 'MAST', logfc.threshold = 0)
FC <- DE$avg_logFC
names(FC) <- toupper(rownames(DE))
PValue <- fgseaMultilevel(KEGG['Apoptosis'], FC)
F <- plotEnrichment(KEGG$Apoptosis, FC) +
  ...
  xlab('Gene rank') +
  ylab('Enrichment Score') +
  labs(title = 'Apoptosis', subtitle = paste0('8 vs 3 | NES = ',round(PValue$NES,2), ' | P = ', formatC(PValue$padj, format = 'e', digits = 2)))

DE <- FindMarkers(downloadedCells, ident.1 = '8', ident.2 = '1', test.use = 'MAST', logfc.threshold = 0)
FC <- DE$avg_logFC
names(FC) <- toupper(rownames(DE))
PValue <- fgseaMultilevel(KEGG['Apoptosis'], FC)
G <- plotEnrichment(KEGG$Apoptosis, FC) +
  ...
  xlab('Gene rank') +
  ylab('Enrichment Score') +
  labs(title = 'Apoptosis', subtitle = paste0('8 vs 1 | NES = ',round(PValue$NES,2), ' | P = ', formatC(PValue$padj, format = 'e', digits = 2)))

DE <- FindMarkers(downloadedCells, ident.1 = '6', ident.2 = '1', test.use = 'MAST', logfc.threshold = 0)
FC <- DE$avg_logFC
names(FC) <- toupper(rownames(DE))
PValue <- fgseaMultilevel(KEGG['Apoptosis'], FC)
